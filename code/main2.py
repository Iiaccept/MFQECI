...
def cross_validation_5fold(k_folds):
    ...
    for f in range(1, k_folds + 1):
        ...
        # auc1, aupr1, recall1, precision1, f11, acc1, mcc1, loss_list, accuracy_list, mcc_list, all_fpr, all_tpr, all_auc, fpr, tpr = train(
        #     testset, epochs)
        # tprs.append(tpr)
        # fprs.append(fpr)
...
if __name__ == '__main__':
    ...
    #数据集1
    # 关联矩阵
    MD = np.loadtxt("dataset2/association.txt")
    # 药物文本相似性矩阵
    S_drug_text = np.loadtxt("dataset2/LLM_drug_sim.txt")
    # RNA文本相似性矩阵
    S_rna_text = np.loadtxt("dataset2/LLM_rna_sim.txt")
    # 药物序列相似性矩阵
    S_drug_seq = np.loadtxt("dataset2/Seq_drug_sim.txt")
    # RNA序列相似性矩阵
    S_rna_seq = np.loadtxt("dataset2/Seq_rna_sim.txt")
    # 药物图像相似性矩阵
    S_drug_img = np.loadtxt("dataset2/Img_drug_sim.txt")
